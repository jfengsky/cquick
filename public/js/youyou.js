...
(function(window, undefined) {
    ...
    attr: function(_key, _value) {
      if (isIE && ieVersion <= 7) {
        if (_value) {
          // set attribute
          if (_key === 'class') {
            this.elements.setAttribute('className', _value)
          } else {

            // 设置其它属性的兼容写法
            var tempAttr = document.createAttribute(_key);
            tempAttr.nodeValue = _value + '';
            this.elements.setAttributeNode(tempAttr);
            return this
          }
        } else {
          // 获取其它属性的兼容写法
          var tempNode = this.elements.getAttributeNode(_key);
          if (tempNode) {
  ...
  var QA = function() {
     ...
     * @return {[type]}      [description]
     */
    this.formatAjaxParams = function(data) {
      var arr = [];
    ...
    this.ajax = function(options) {
      ...
      var params = this.formatAjaxParams(options.data);
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
          ...
          if (status >= 200 && status < 300) {
            if (isIE && ieVersion <= 7) {
              if (xhr.responseText.indexOf('function') === -1) {
                resText = eval("(" + xhr.responseText + ")");
              }
            } else {
              resText = JSON.parse(xhr.responseText)
            };
            options.success && options.success(resText, xhr.responseXML);
          } else {
    ...
    */
    ...
    this._scrollBottom = function() {
      var scrollCnt = doc.getElementById('J_bodysearch'),
        distance = 99999 + (questionIndex * 400);

      if (scrollCnt) {
        scrollCnt.scrollTop = distance
      } else {
    ...
    this._fuzzyTpl = function(_index, _resTime, _searchResult, _searchResultKeyWord) {
      ...
      if (fuzzyAnswer && fuzzyAnswer.length) {
        for (var i = 0; i < fuzzyAnswer.length; i++) {
          ...
          answerStr += '<div class="visa_box"><div class="vbk_ctrip_info ' + lightClass + '"><div class="you_content content_exceed">' + this._lightKeyword(fuzzyAnswer[i].R, KeyWordArr) + ' [' + fuzzyAnswer[i].SN + ', ' + fuzzyAnswer[i].S + ']</div><a class="exceed_btn" href="###" style="display:none">查看更多</a>' + feedbackTpl + '</div></div>'
        };
    ...
    this._sendData = function(_options) {
      ...
      if (sendType === 'ajax') {
        this.ajax(sendOption)
      } else {
        this.jsonp(sendOption)
      }
    ...
    this._offlineClick = function() {
      ...
      if (isShow === 'true') {
        ...
        if (isIE && ieVersion <= 7) {
          content.setAttribute('className', 'youyou_box');
          content.style.position = 'absolute';
          content.style.left = '10px';
          content.style.bottom = 0;
        } else {
          content.setAttribute('class', 'youyou_box');
          content.setAttribute('style', 'position:fixed;_position:absolute;bottom:0;left:10px;z-index:1000');
        }
    ...
    this._drag = function() {
      ...
      $(document).bind('mousemove.youyou', function(ev) {
    ...
    this._boxInit = function() {
      ...
      $G('J_minyouyou').bind('click', this._offlineClick);
      ...
      if (isIE && ieVersion <= 7) {
        $G(window).bind('scroll', function() {
          var disTop = document.documentElement.scrollTop + (document.documentElement.clientHeight - 602);
          $G('J_youyou_box').elements.style.top = disTop + 'px';
        })
      };
      ...
      $(document).delegate('.exceed_btn', 'click', function() {
        ...
        if (!tag || tag === 'show') {
          ...
          // 收起
          $(this).text('查看更多').prev().addClass('content_exceed');
